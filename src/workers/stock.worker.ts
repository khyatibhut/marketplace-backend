...
export const initStockWorker = () => {
  const worker = new Worker(
    ...
    async (job: Job) => {
      ...
      console.log(`[Stock Worker] Processing stock restoration for cancelled order ${orderId}`);
      ...
      try {
        ...
        console.log(`[Stock Worker] Successfully restored stock for order ${orderId}`);
      } catch (err) {
        await session.abortTransaction();
        console.error(`[Stock Worker] DB Transaction failed restoring stock for ${orderId}`, err);
        throw err;
      } finally {
        session.endSession();
      }
