...
const invalidateProductCaches = async (productId?: string) => {
  ...
  if (productId)
    tasks.push(
      deleteCache(PRODUCT_KEY(productId)),
      deleteCache(STOCK_KEY(productId)),
    );
...
export const getProducts = async (req: Request, res: Response) => {
  try {
    ...
    const cacheKey = PRODUCTS_LIST_KEY(
    ...
    if (cached)
      return sendSuccess(
        res,
        200,
        "Products listed successfully (cached)",
        cached,
      );
  ...
  try {
    ...
    if (cached)
      return sendSuccess(
        res,
        200,
        "Product details fetched successfully (cached)",
        { product: cached },
      );
  ...
  try {
    ...
    if (cached)
      return sendSuccess(
        res,
        200,
        "Product stock fetched successfully (cached)",
        cached,
      );
...
export const updateProduct = async (req: Request, res: Response) => {
  try {
    ...
    if (!product)
      return sendError(res, 404, "Product not found or unauthorized to update");
...
export const deleteProduct = async (req: Request, res: Response) => {
  try {
    ...
    if (!product)
      return sendError(res, 404, "Product not found or unauthorized to delete");
