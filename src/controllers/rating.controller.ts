  ...
  try {
    ...
    if (!order) {
      return sendError(
        res,
        403,
        "You can only rate products from your own delivered orders",
      );
    }

    const itemInOrder = order.items.some(
      (i) => i.productId.toString() === validatedData.productId,
    );
    if (!itemInOrder) {
      return sendError(
        res,
    ...
    if (existing) {
      return sendError(
        res,
        409,
        "You have already rated this product for this order",
      );
    }
...
export const getProductRatings = async (req: Request, res: Response) => {
  try {
    ...
    const averageScore = Number(
      (result.metadata[0]?.averageScore || 0).toFixed(1),
    );
