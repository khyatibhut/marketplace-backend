...
// ─── Cache key builders (complex reads only) ────────────────────────────────
const ORDER_KEY        = (id: string)                  => `order:${id}`;
const ORDER_STATUS_KEY = (id: string)                  => `order:status:${id}`;
const BUYER_ORDERS_KEY = (buyerId: string, qs: string) => `orders:buyer:${buyerId}:${qs}`;
...
const invalidateOrderCaches = async (orderId?: string, buyerId?: string) => {
  ...
  if (orderId) tasks.push(deleteCache(ORDER_KEY(orderId)), deleteCache(ORDER_STATUS_KEY(orderId)));
...
export const createOrder = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Validate each item: product exists + sufficient stock
    let totalAmount = 0;
    const orderItems: { productId: mongoose.Types.ObjectId; quantity: number; pricePerItem: number }[] = [];
    ...
    for (const item of items) {
      ...
      if (product.stock < item.quantity) {
        await session.abortTransaction();
        return sendError(res, 409, `Insufficient stock for "${product.name}" (available: ${product.stock})`);
      }
...
export const getMyOrders = async (req: Request, res: Response) => {
  try {
    ...
    const cacheKey = BUYER_ORDERS_KEY(req.user.id, new URLSearchParams(req.query as any).toString());

    const cached = await getCache(cacheKey);
    if (cached) return sendSuccess(res, 200, 'Orders fetched successfully (cached)', cached);
...
export const getOrderDetails = async (req: Request, res: Response) => {
  try {
    ...
    const cached = await getCache(cacheKey);
    if (cached) return sendSuccess(res, 200, 'Order details fetched successfully (cached)', { order: cached });
...
export const getOrderStatus = async (req: Request, res: Response) => {
  try {
    ...
    const cached = await getCache<{ status: string; statusHistory: unknown[] }>(cacheKey);
    if (cached) return sendSuccess(res, 200, 'Order status fetched successfully (cached)', cached);
    ...
    const payload = { status: order.status, statusHistory: order.statusHistory };
...
export const cancelOrder = async (req: Request, res: Response) => {
  ...
  session.startTransaction();
  try {
    const order = await Order.findOne({ _id: req.params.id, buyerId: req.user.id }).session(session);
    if (!order) { await session.abortTransaction(); return sendError(res, 404, 'Order not found'); }
...
export const getSellerOrderDetails = async (req: Request, res: Response) => {
  try {
    const order = await Order.findOne({ _id: req.params.id, sellerIds: req.user.id })
      .populate('buyerId', 'name email')
...
export const updateOrderStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (!allowed.includes(status as OrderStatus)) {
      return sendError(res, 409, `Cannot transition from "${order.status}" to "${status}"`);
    }
...
export const resetStuckOrders = async (req: Request, res: Response) => {
  try {
    ...
    const stuckStatuses = [OrderStatus.CONFIRMED, OrderStatus.PREPARING, OrderStatus.OUT_FOR_DELIVERY];
    ...
    sendSuccess(res, 200, 'Stuck orders reset successfully', { modifiedCount: result.modifiedCount });
  } catch (error: any) {
...
export const getStatistics = async (req: Request, res: Response) => {
  try {
    ...
    const cached = await getCache(cacheKey);
    if (cached) return sendSuccess(res, 200, 'Statistics fetched successfully (cached)', cached);
